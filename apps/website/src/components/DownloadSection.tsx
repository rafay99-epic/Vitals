...
import { BREW_INSTALL, DOWNLOAD_URL, REPO_URL } from '../lib/links'
  ...
  return (
    <section id="download" style={{ position: 'relative', padding: '70px 24px 60px', display: 'flex', justifyContent: 'center' }}>
      ...
      >
        ...
        <p style={{ fontSize: 17, lineHeight: 1.5, color: 'rgba(235,235,245,0.6)', margin: '0 auto 30px', maxWidth: '44ch' }}>
          Free and open source. One command installs it to Applications — no security prompt, and it keeps itself up to date.
        </p>

        {/* Primary: Homebrew. Opens cleanly (the cask strips the download quarantine). */}
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 9 }}>
          <CommandBox command={BREW_INSTALL} />
          <span style={{ fontSize: 12.5, color: 'rgba(235,235,245,0.45)' }}>
            Recommended — installs via Homebrew, no “unverified developer” prompt.
          </span>
        </div>

        {/* Secondary: direct DMG + GitHub. */}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, flexWrap: 'wrap', marginTop: 26 }}>
          <a href={DOWNLOAD_URL} style={ghostButton}>
            <svg aria-hidden="true" width="16" height="16" viewBox="0 0 24 24" fill="#f5f5f7">
          ...
          <a href={REPO_URL} target="_blank" rel="noreferrer" style={ghostButton}>
            <svg aria-hidden="true" width="16" height="16" viewBox="0 0 24 24" fill="#f5f5f7">
        ...
        <p style={{ fontSize: 11.5, color: 'rgba(235,235,245,0.34)', margin: '10px auto 0', maxWidth: '40ch' }}>
          The .dmg isn’t notarized — right-click → Open the first time to get past Gatekeeper.
        </p>
