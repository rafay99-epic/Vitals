...
final class VitalsModelTests: XCTestCase {
    ...
    func testHistoryGrowsOnlyWithUsage() {
        var history: [VitalsModel.Point] = []
        // First tick: no usage delta yet → no history point.
        _ = VitalsModel.derive(snapshot(cpuTemps: [45], usage: nil), history: &history)
        XCTAssertEqual(history.count, 0)
        // Subsequent ticks with usage → history accumulates.
    ...
    func testHistoryIsCappedAndDownsampled() {
        ...
        XCTAssertLessThanOrEqual(history.count, VitalsModel.maxHistory)
