...
struct HelpView: View {
    ...
    private static let topics: [Topic] = [
        ...
        Topic(
            ...
            tint: .purple,
            title: "Applications",
            body: "Two segments: Installed and Startup. Installed includes app bundles by default. Enable Settings → Applications → Scan CLI and system applications to also include globally managed CLI tools and macOS system apps. Uninstalling an app moves its leftover files (caches, preferences, containers, launch agents) to the Trash — nothing is deleted permanently, so you can always recover from the Finder. CLI tools are removed by their owning package manager. Protected system apps, Apple software, and package-manager dependencies remain visible but cannot be selected; unowned or manually copied binaries are never guessed. Startup shows everything that launches itself at login; you can disable or remove your own items, while Apple's stay read-only."
        ),
        Topic(
            symbol: "sparkles",
            tint: .orange,
