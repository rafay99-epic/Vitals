...
/// and works on Apple Silicon. The GPU's name and total memory come from Metal
/// and never change during a run, so they're captured once.
///
...
final class GPUSampler {
    ...
    func sample() -> GPUSnapshot? {
        ...
        return GPUSnapshot(
            name: name,
            utilization: utilization.map { min(max(Double($0), 0), 100) },
